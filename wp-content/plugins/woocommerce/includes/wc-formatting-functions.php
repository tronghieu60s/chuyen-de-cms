...
function wc_price( $price, $args = array() ) {
	...
	$formatted_price = ( $negative ? '-' : '' ) . sprintf( $args['price_format'], '<span class="woocommerce-Price-currencySymbol">' . 'VND' . '</span>', $price );
	$return          = '<span class="woocommerce-Price-amount amount custom"><bdi>' . $formatted_price . '</bdi></span>';
...
function wc_format_sale_price( $regular_price, $sale_price ) {
	$price = '<del aria-hidden="true">' . ( is_numeric( $regular_price ) ? wc_price( $regular_price ) : $regular_price ) . '</del>' . ( is_numeric( $sale_price ) ? wc_price( $sale_price ) : $sale_price ) ;
